...
import { JobOrder, JobOrderMaterial, JobOrderType } from '@/types/job-order';
import { Material } from '@/types/material';
import { Product } from '@/types/product';
import { Color } from '@/types/color';
import { Size } from '@/types/size';

interface UpdateJobOrderFormProps {
	isOpen: boolean;
	onClose: () => void;
	onSuccess: () => void;
...
const mapJobMaterials = (materials?: JobOrderMaterial[]): MaterialRow[] => {
...
export default function UpdateJobOrderForm({
	isOpen,
	onClose,
	onSuccess,
	jobOrder,
}: UpdateJobOrderFormProps) {
		...
		products.forEach((product) => {
			...
			variants.forEach((variant, index) => {
				...
				options.push({
					...
					label: `${product.product_name} | ${sizeName} | ${colorName} | Variant ${index + 1}`,
	...
	useEffect(() => {
		...
		setMaterials(mapJobMaterials(jobOrder.jobOrderMaterials || jobOrder.job_order_materials));
		setErrors({});
	}, [isOpen, jobOrder]);
	...
	const handleSubmit = async (event: React.FormEvent) => {
		...
		try {
			await jobOrderModel.updateJobOrder({
			...
			setResultDialog({
				...
				status: 'error',
				action: 'update',
				message: error instanceof Error ? error.message : 'เกิดข้อผิดพลาดในการอัปเดตงานผลิต',
			});
		} finally {
