...
interface InsertBankAccountFormProps {
	isOpen: boolean;
	onClose: () => void;
	onSuccess: () => void;
}
...
export default function InsertBankAccountForm({ isOpen, onClose, onSuccess }: InsertBankAccountFormProps) {
	...
	const handleSubmit = async (event: React.FormEvent) => {
		...
		try {
			...
			setResultDialog({
				isOpen: true,
				status: 'success',
				message: 'เพิ่มบัญชีรับเงินสำเร็จ',
			});
		} catch (error) {
			setResultDialog({
				isOpen: true,
				status: 'error',
				message: error instanceof Error ? error.message : 'เกิดข้อผิดพลาดในการเพิ่มบัญชีรับเงิน',
			});
		} finally {
			...
			<div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4 backdrop-blur-sm">
				<div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl dark:bg-gray-800">
					<div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
						<h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">เพิ่มบัญชีรับเงิน</h2>
					</div>
