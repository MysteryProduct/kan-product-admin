...
export default function UpdatePurchaseOrderForm({
    ...
    useEffect(() => {
        if (isOpen && initialData) {
            ...
            setSupplierId(initialData.supplier_id || initialData.supplier?.supplier_id || '');
            setVatType(initialData.vat_type || 'none');

            const initialItems = ((initialData.purchaseOrderLists ?? (initialData as any).items ?? []) as PurchaseOrderItem[]);
    ...
    const getMaterialOptionsForItem = (itemId: string) => {
        const selectedInOtherRows = new Set(
            items
                .filter((item) => item.id !== itemId && item.material_id)
    ...
    const validateForm = () => {
        ...
        items.forEach((item) => {
            ...
            materialUsage.set(item.material_id, (materialUsage.get(item.material_id) || 0) + 1);
        });

        items.forEach((item, index) => {
            if (item.material_id && (materialUsage.get(item.material_id) || 0) > 1) {
                newErrors[`item_${index}_material`] = 'วัตถุดิบซ้ำ กรุณาเลือกวัตถุดิบที่ไม่ซ้ำกัน';
            }
        });
